...
function getVehicleCategoria(v) {
  ...
  // Default: auto non riconosciuta → CHIUSA
...
const VehicleIcon = memo(function VehicleIcon({ type, className = 'w-5 h-5' }) {
  ...
  if (type === 'ebike' || type === 'bicicletta') return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className={className} aria-hidden="true">
      <circle cx="6" cy="17" r="3" /><circle cx="18" cy="17" r="3" />
      <path d="M6 17L10 7L13 7" /><path d="M18 17L14 10L11 10" /><path d="M14 4L13 7L15 7L14 10" strokeWidth="2.2" />
      {/* fulmine e-bike */}
      <path d="M11 2L9 7H12L10 12" strokeWidth="1.4" stroke="#f59e0b" />
    </svg>
  );
  if (type === 'bici') return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className={className} aria-hidden="true">
      <circle cx="6" cy="17" r="3" /><circle cx="18" cy="17" r="3" />
      <path d="M6 17L10 7L13 7" /><path d="M18 17L14 10L11 10" /><path d="M10 7L14 10" />
    </svg>
  );
  return null;
});
...
// Conta i veicoli per tipo — usa canonicalTipo per normalizzare alias RentMe
function useFleetCounts(fleet) {
  return useMemo(() => {
    const counts = { auto: 0, scooter: 0, quad: 0, ebike: 0, bici: 0 };
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--ink-2)', marginBottom: 4 }}>
          ...
          {p.vehicleId && p.vehicleId !== p.rentmeCode && (
            <span style={{ fontFamily: 'monospace', fontSize: 11, color: 'var(--muted)', marginLeft: 4 }}>
              {p.vehicleId}
            </span>
          )}
          {p.rentmeCode && (
            <span style={{ fontSize: 10, color: 'var(--muted)', marginLeft: 2, opacity: 0.7 }}>
              [{p.rentmeCode}]
            </span>
          )}
          {p.clienteTel && <>
        ...
        {/* Note interne — evidenziate in arancione */}
        {p.noteInterne && (
        ...
        {p.vehicleSchedule && p.vehicleSchedule.length > 1 && (
          <div style={{ marginTop: 5, display: 'flex', flexWrap: 'wrap', gap: 3, alignItems: 'center' }}>
            <span style={{ fontSize: 10, color: 'var(--muted)', opacity: 0.8 }}>🔄 cambio mezzo:</span>
            {p.vehicleSchedule.map((s, i) => (
              <span key={i} style={{ fontSize: 10, background: 'rgba(31,93,131,.1)', color: '#1f5d83',
                borderRadius: 3, padding: '1px 6px', fontFamily: 'monospace', fontWeight: 600 }}>
                {s.vehicleId}
                <span style={{ fontWeight: 400, color: 'var(--muted)', marginLeft: 2 }}>
                  {formatDate(s.dal)}→{formatDate(s.al)}
                </span>
...
// ═══════════════════════════════════════════════════════════════════
// SMART ASSIGNMENT — trova la combinazione ottimale di mezzi
// per coprire un periodo quando nessun singolo mezzo è disponibile.
// Algoritmo greedy: assegna il mezzo che copre più giorni dall'inizio,
// poi continua con il successivo libero, e così via.
      ...
      <div style={{ background: 'var(--bg)', borderRadius: 10, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto', padding: 24 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            ...
            {/* Smart combo: nessun mezzo singolo disponibile → propone combinazione */}
            {!f.vehicleId && f.vehicleType && f.dal && f.al && smartCombo && !smartCombo.impossible && (
              <div style={{ marginTop: 8, padding: '10px 12px', borderRadius: 6,
                background: '#f0f4ff', border: '1px solid #aac0f0', fontSize: 12 }}>
                <div style={{ fontWeight: 700, color: '#1f5d83', marginBottom: 6 }}>
                  🧩 Nessun mezzo singolo disponibile — combinazione automatica trovata:
                </div>
                {smartCombo.map((seg, i) => (
                  <div key={seg.vehicleId} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, color: '#1f5d83' }}>
                    <span style={{ fontWeight: 600, fontFamily: 'monospace', fontSize: 11 }}>{seg.vehicleId}</span>
                    <span style={{ fontSize: 11, color: 'var(--muted)' }}>
                      {formatDate(seg.dal)} → {formatDate(seg.al)}
                    </span>
                    {seg.vehicle && <span style={{ fontSize: 10, color: 'var(--muted)' }}>({seg.vehicle.modello || seg.vehicle.tipo})</span>}
                  </div>
                ))}
                <button type="button"
                  onClick={() => {
                  ...
                  style={{ marginTop: 8, padding: '6px 14px', borderRadius: 5, border: 'none',
                    background: '#1f5d83', color: 'white', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
                  ✓ Usa questa combinazione
                </button>
              </div>
            )}
            {!f.vehicleId && f.vehicleType && f.dal && f.al && smartCombo?.impossible && (
              <div style={{ marginTop: 8, padding: '10px 12px', borderRadius: 6,
                background: '#fff0f0', border: '1px solid #f0b0b0', fontSize: 12, color: '#c85050' }}>
                ⛔ <strong>Impossibile coprire il periodo</strong> · nessuna combinazione di {f.vehicleType} disponibile.
                Considera date diverse o un tipo diverso.
              </div>
            )}
            {/* Preview schedule attivo */}
            {vehicleSchedule && vehicleSchedule.length > 1 && (
              <div style={{ marginTop: 8, padding: '8px 12px', borderRadius: 6,
                background: '#e8f5e9', border: '1px solid #a5d6a7', fontSize: 11 }}>
                <span style={{ fontWeight: 700, color: '#1b5e20' }}>🔄 Cambio mezzo programmato:</span>
                {vehicleSchedule.map((s, i) => (
                  <span key={s.vehicleId} style={{ marginLeft: 6, color: '#2e6e3e' }}>
                    {s.vehicleId} ({formatDate(s.dal)}→{formatDate(s.al)}){i < vehicleSchedule.length - 1 ? ' →' : ''}
                  </span>
                ))}
                <button type="button" onClick={() => { setVehicleSchedule(null); set('vehicleId', ''); }}
                  style={{ marginLeft: 10, fontSize: 10, color: '#c85050', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>
                  rimuovi
                </button>
              </div>
            )}
          ...
          <div>
            <label style={lbl}>
              Note cliente
              <span style={{ fontWeight: 400, color: 'var(--muted)', marginLeft: 6, fontSize: 10 }}>visibili sul contratto</span>
            </label>
            <textarea style={{ ...inp, resize: 'vertical', minHeight: 50 }} value={f.noteCliente} onChange={e => set('noteCliente', e.target.value)} placeholder="Richieste speciali, preferenze, accordi…" />
          </div>

          {/* Note interne (solo operatori) */}
          <div>
            <label style={lbl}>
              Note interne
              <span style={{ fontWeight: 400, color: 'var(--accent)', marginLeft: 6, fontSize: 10 }}>🔒 solo operatori</span>
            </label>
...
function PrenotazioniPage({ prenotazioni, setPrenotazioni, setCassa, fleet, rentmeVehicles, customers, operator, onOpenWizard, pushToast, prefill, onClearPrefill, fermiFlotta, rentmePush, rentmeConnected }) {
  ...
  // focusId: ID prenotazione da mettere in evidenza (arriva da Calendario)
  ...
  useEffect(() => {
    ...
    if (prefill.focusId && !prefill.vehicleId && !prefill.vehicleLabel) {
      ...
      setForm({ ...prefill, id: '__new__' }); // id fittizio che segnala "è una nuova prenotazione con prefill"
      onClearPrefill && onClearPrefill();
    }
  }, [prefill]);

  // Scroll + highlight: appena focusId cambia e il DOM si aggiorna
  useEffect(() => {
    if (!focusId) return;
    const timer = setTimeout(() => {
      const el = document.getElementById(`preno-card-${focusId}`);
      if (el) {
        el.scrollIntoView({ behavior: 'smooth', block: 'center' });
        el.classList.add('preno-highlight');
        setTimeout(() => el.classList.remove('preno-highlight'), 2200);
      }
      setFocusId(null);
    }, 120); // piccolo ritardo per lasciar rendere la lista
  ...
  const [sostituzionePreno, setSostituzionePreno] = useState(null); // prenotazione da sostituire
  const [consegnaPreno, setConsegnaPreno] = useState(null);     // prenotazione da consegnare
  const [riconsegnaPreno, setRiconsegnaPreno] = useState(null); // prenotazione da riconsegnare
  ...
  function handleConsegna({ vehicleId, vehicleLabel, vehicleSchedule, kmPartenza, carburante, noteConsegna }) {
    const p = consegnaPreno;
    setPrenotazioni(ps => ps.map(x => x.id === p.id ? {
      ...x,
      stato:           'in_corso',
      vehicleId:       vehicleId       || x.vehicleId    || null,
      vehicleLabel:    vehicleLabel    || x.vehicleLabel || '',
      vehicleSchedule: vehicleSchedule || x.vehicleSchedule || null,
  ...
  function handleRiconsegna({ kmRientro, carburanteRientro, danniRiscontrati, noteDanni, noteRientro }) {
    const p = riconsegnaPreno;
    const hasDanni = danniRiscontrati && danniRiscontrati.length > 0;
    setPrenotazioni(ps => ps.map(x => x.id === p.id ? {
      ...x,
      stato:              hasDanni ? 'completata' : 'completata',
      rientroEffettivo:   todayISO(),
      kmRientro:          kmRientro != null ? kmRientro : (x.kmRientro ?? null),
      carburanteRientro:  carburanteRientro || null,
      danniRiscontrati:   danniRiscontrati || null,
      noteDanni:          noteDanni || '',
      noteRientro:        noteRientro || '',
      riconsegnaAt:       new Date().toISOString(),
      updatedAt:          new Date().toISOString(),
    } : x));
    setRiconsegnaPreno(null);
    pushToast && pushToast({
      tone: hasDanni ? 'warning' : 'success',
      title: hasDanni ? '⚠ Rientro con danni' : '📍 Riconsegna completata',
      message: [
        [p.clienteCognome, p.clienteNome].filter(Boolean).join(' '),
        hasDanni ? `Danni: ${danniRiscontrati.join(', ')}` : null,
      ].filter(Boolean).join(' · '),
    });
  }
  ...
  function applySostituzione({ dataGuasto, motivazione, noteExtra, mezzoNuovo, candidati }) {
    const groupId = `sost_${Date.now()}`;
    // Giorno prima del guasto = nuova fine prenotazione originale
    const finePrima = new Date(dataGuasto + 'T12:00:00');
    finePrima.setDate(finePrima.getDate() - 1);
    const fineISO = finePrima.toISOString().slice(0, 10);
    const p = sostituzionePreno;
    const sostituto = (candidati || []).find(v => v.id === mezzoNuovo);
    const labelSost = sostituto
      ? `${sostituto.modello || sostituto.tipo || ''} · ${sostituto.targa || mezzoNuovo}`.trim()
      : mezzoNuovo;

    setPrenotazioni(ps => {
      const updated = ps.map(x => x.id === p.id ? {
        ...x, al: fineISO, stato: 'completata',
        note: [x.note, `Sostituzione ${motivazione} il ${dataGuasto}${noteExtra ? ' — ' + noteExtra : ''}`].filter(Boolean).join(' | '),
        sostituzioneGroupId: groupId,
        updatedAt: new Date().toISOString(),
      } : x);
      const nuova = {
        id: `preno_${Date.now()}_sost`,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
        ...
        stato: 'in_corso',
        fonte: 'sostituzione',
        prezzo: null, acconto: null,
        note: `Sostituzione di ${p.vehicleLabel || p.vehicleId} (${motivazione})${noteExtra ? ' — ' + noteExtra : ''}`,
        sostituzioneGroupId: groupId,
        sostituzioneOriginaleId: p.id,
      };
      return [...updated, nuova];
    });
    pushToast && pushToast({ tone: 'success', title: '🔧 Sostituzione effettuata',
      message: `${labelSost} · dal ${formatDate(dataGuasto)} al ${formatDate(p.al)}` });
    setSostituzionePreno(null);
  }
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 860, margin: '0 auto' }}>
      ...
      {form && (
        <PrenoForm
          initial={form === 'new' || form?.id === '__new__' ? null : form}
          fleet={fleet}
      ...
      {riconsegnaPreno && (
        <RiconsegnaModal
          preno={riconsegnaPreno}
          onConfirm={handleRiconsegna}
          onClose={() => setRiconsegnaPreno(null)}
        />
      )}
      ...
      {saldoPreno && (
        <SaldoModal
          preno={saldoPreno}
          operator={operator}
          onSave={({ record, completata }) => {
            // Registra movimento in cassa
            setCassa && setCassa(prev => [...(prev || []), record]);
            // Aggiorna prenotazione: saldoRegistrato + eventuale completata
            setPrenotazioni(ps => ps.map(p => p.id === saldoPreno.id ? {
              ...p,
              saldoRegistrato: true,
              stato: completata ? 'completata' : p.stato,
              updatedAt: new Date().toISOString(),
            } : p));
            setSaldoPreno(null);
            pushToast && pushToast({ tone: 'success', title: '💰 Saldo registrato',
              message: `€${record.importo} · ${record.metodo}` });
          }}
          onClose={() => setSaldoPreno(null)}
        />
      )}
      ...
      {depositoPreno && (
        <DepositoModal
          preno={depositoPreno.preno}
          operator={operator}
          mode={depositoPreno.mode}
          onSave={({ record, importo: imp }) => {
            const isRimborso = depositoPreno.mode === 'rimborsa';
            // Registra movimento in cassa
            setCassa && setCassa(prev => [...(prev || []), record]);
            // Aggiorna prenotazione
            setPrenotazioni(ps => ps.map(p => p.id === depositoPreno.preno.id ? {
              ...p,
              ...(isRimborso
                ? { depositoRimborsato: true }
                : { depositoRegistrato: true, depositoImporto: imp }),
              updatedAt: new Date().toISOString(),
            } : p));
...
// ═══════════════════════════════════════════════════════════════════
// SOSTITUZIONE VEICOLO — emergenza guasto / incidente
...
function SostituzioneModal({ preno, prenotazioni, fleet, rentmeVehicles, onConfirm, onClose }) {
  ...
  const [mezzoNuovo,  setMezzoNuovo]  = useState('');

  // Mezzi dello stesso tipo liberi dal giorno del guasto alla fine prenotazione
  ...
  const inp = {
    ...
    fontSize: 13, background: 'var(--bg)', color: 'var(--ink)',
    fontFamily: 'var(--font-sans)', width: '100%', boxSizing: 'border-box',
  };

  const giorni = dataGuasto && preno.al
    ? Math.max(0, Math.round((new Date(preno.al + 'T12:00:00') - new Date(dataGuasto + 'T12:00:00')) / 86400000))
    : 0;
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 700,
      ...
      <div style={{ background: 'var(--bg)', borderRadius: 12, padding: '24px 28px',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 17, fontFamily: 'var(--font-serif)', fontWeight: 700, color: '#c85050' }}>
              🔧 Sostituzione veicolo
            </h2>
            <p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--muted)' }}>
              {[preno.clienteCognome, preno.clienteNome].filter(Boolean).join(' ')} · {preno.vehicleLabel || preno.vehicleId}
            </p>
          </div>
        ...
        {/* Info prenotazione originale */}
        <div style={{ background: 'var(--surface-2)', borderRadius: 7, padding: '10px 14px',
          marginBottom: 18, fontSize: 12, color: 'var(--ink-2)' }}>
          <strong>Prenotazione originale:</strong> {formatDate(preno.dal)} → {formatDate(preno.al)}
          {preno.vehicleId && <> · <span style={{ fontFamily: 'monospace' }}>{preno.vehicleId}</span></>}
        </div>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>
            La prenotazione originale verrà chiusa il giorno prima ({dataGuasto
              ? (() => { const d = new Date(dataGuasto + 'T12:00:00'); d.setDate(d.getDate()-1); return formatDate(d.toISOString().slice(0,10)); })()
              : '—'
            })
          </div>
        </div>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <select value={motivazione} onChange={e => setMotivazione(e.target.value)} style={inp}>
            <option value="guasto">Guasto meccanico</option>
            <option value="incidente">Incidente</option>
            <option value="foratura">Foratura / problema pneumatico</option>
            <option value="altro">Altro</option>
          </select>
        </div>
        ...
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: 'block', fontSize: 11, fontWeight: 700, textTransform: 'uppercase',
            letterSpacing: '.06em', color: 'var(--muted)', marginBottom: 6 }}>
            Mezzo sostitutivo — libero {formatDate(dataGuasto)} → {formatDate(preno.al)} ({giorni}gg)
          </label>
          {candidati.length === 0 ? (
            <div style={{ padding: '10px 14px', borderRadius: 6, background: '#fff0f0',
              border: '1px solid #f0b0b0', fontSize: 12, color: '#c85050' }}>
              ⚠️ Nessun mezzo dello stesso tipo disponibile per questo periodo.
              Puoi scegliere una categoria diversa modificando manualmente dopo.
            </div>
          ) : (
            <select value={mezzoNuovo} onChange={e => setMezzoNuovo(e.target.value)} style={inp}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <input value={noteExtra} onChange={e => setNoteExtra(e.target.value)}
            placeholder="Es. cliente avvisato, mezzo in officina..."
            style={inp} />
        </div>

        {/* Anteprima risultato */}
        {mezzoNuovo && (
          <div style={{ background: '#e8f5e9', border: '1px solid #a5d6a7', borderRadius: 7,
            padding: '10px 14px', marginBottom: 18, fontSize: 12, color: '#1b5e20' }}>
            <div style={{ fontWeight: 700, marginBottom: 4 }}>✓ Cosa verrà creato:</div>
            <div>1. Prenotazione originale <strong>chiusa</strong> il {
              (() => { const d = new Date(dataGuasto + 'T12:00:00'); d.setDate(d.getDate()-1); return formatDate(d.toISOString().slice(0,10)); })()
            } · nota: {motivazione}</div>
            <div style={{ marginTop: 3 }}>2. Nuova prenotazione su <strong>{mezzoNuovo}</strong> dal {formatDate(dataGuasto)} al {formatDate(preno.al)} — stesso cliente</div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          ...
          <button
            disabled={!mezzoNuovo || !dataGuasto}
            onClick={() => onConfirm({ dataGuasto, motivazione, noteExtra, mezzoNuovo, candidati })}
            style={{ padding: '8px 20px', borderRadius: 6, border: 'none',
              background: !mezzoNuovo ? 'var(--border)' : '#c85050',
              color: !mezzoNuovo ? 'var(--muted)' : 'white',
              fontWeight: 700, cursor: !mezzoNuovo ? 'not-allowed' : 'pointer', fontSize: 13 }}>
...
// ═══════════════════════════════════════════════════════════════════
// CONSEGNA VEICOLO — assegnazione targa + km + carburante
// Apre quando si clicca "🚗 Consegna" su una prenotazione confermata.
...
function ConsegnaModal({ preno, prenotazioni, fleet, rentmeVehicles, onConfirm, onClose }) {
  ...
  const vehicleFitScore = useMemo(() => {
    ...
    candidates.forEach(v => {
      ...
      // Conta i giorni liberi consecutivi dalla data di partenza
      let cursor = dalDate;
      let freeCount = 0;
      while (cursor <= alDate) {
  ...
  const candidati = useMemo(() => {
    return allVehicles
      ...
      .sort((a, b) => {
        // -1 = occupato → in fondo
        if (a.freeScore < 0 && b.freeScore >= 0) return 1;
        if (b.freeScore < 0 && a.freeScore >= 0) return -1;
        // Perfetto = copre esattamente il periodo → priorità assoluta
        const aPerfect = a.freeScore >= nNoleggio;
        const bPerfect = b.freeScore >= nNoleggio;
        if (aPerfect && !bPerfect) return -1;
        if (bPerfect && !aPerfect) return 1;
        // Altrimenti: più giorni liberi → meglio
        return b.freeScore - a.freeScore;
      });
  }, [allVehicles, vehicleFitScore, tipo, nNoleggio]);

  // Combo smart se nessun singolo mezzo copre l'intero periodo
  const smartCombo = useMemo(() => {
    if (!tipo || !preno.dal || !preno.al) return null;
    const hasSingle = candidati.some(v => v.freeScore >= nNoleggio);
    if (hasSingle) return null;
    return findBestVehicleCombination(tipo, preno.dal, preno.al, allVehicles, prenotazioni, preno.id);
  }, [candidati, tipo, preno, allVehicles, prenotazioni, nNoleggio]);

  // Stato locale
  const [selectedId,     setSelectedId]     = useState(preno.vehicleId || '');
  ...
  const coversAll   = selVehicle ? selVehicle.freeScore >= nNoleggio : false;
  const canConfirm  = useCombo ? (smartCombo && !smartCombo.impossible) : (selectedId && isAvailable && coversAll);

  const FUEL_OPTS = [
    { val: '100', label: '🟢 Pieno',   color: '#2e6e3e' },
    { val: '75',  label: '🟡 3/4',     color: '#8a7a1a' },
    { val: '50',  label: '🟠 Metà',    color: '#c88a2e' },
    { val: '25',  label: '🔴 1/4',     color: '#c85050' },
  ];

  const inp = { background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6,
    padding: '8px 10px', fontSize: 13, color: 'var(--ink)', width: '100%', boxSizing: 'border-box' };
      ...
      <div style={{ background: 'var(--bg)', borderRadius: 12, width: '100%', maxWidth: 640,
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              ...
              <div style={{ fontSize: 13, color: 'var(--muted)', marginTop: 3 }}>
                ...
                {preno.clienteTel && <> · <a href={`tel:${preno.clienteTel}`} style={{ color: 'var(--accent)' }}>{preno.clienteTel}</a></>}
              </div>
              <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>
                {formatDate(preno.dal)} → {formatDate(preno.al)} &nbsp;·&nbsp;
                <strong>{nNoleggio} {nNoleggio === 1 ? 'giorno' : 'giorni'}</strong>
                {tipo && <> &nbsp;·&nbsp; <span style={{ textTransform: 'capitalize' }}>{tipo}</span></>}
              </div>
            </div>
            <button type="button" onClick={onClose}
        ...
        <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div>
            ...
            {/* Griglia veicoli disponibili */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(145px, 1fr))', gap: 8 }}>
              {candidati.slice(0, 18).map(v => {
                const free  = v.freeScore;
                const isSel = v.id === selectedId && !useCombo;
                const covers = free >= nNoleggio;
                const partial = free > 0 && free < nNoleggio;
                const busy  = free < 0;
                ...
                if (busy)    { borderColor = '#e0c0c0'; bgColor = '#fff5f5'; scoreLabel = 'Occupato'; scoreColor = '#c85050'; }
                else if (covers)  { borderColor = '#4a9e5c'; bgColor = '#f0f9f2'; scoreLabel = `✓ ${free}gg liberi`; scoreColor = '#2e6e3e'; }
                else if (partial) { borderColor = '#c88a2e'; bgColor = '#fffbf0'; scoreLabel = `⚠ ${free}/${nNoleggio}gg`; scoreColor = '#c88a2e'; }
                else { scoreLabel = `${free}gg liberi`; scoreColor = 'var(--muted)'; }

                if (isSel) { borderColor = '#1f5d83'; bgColor = '#e8f2f9'; }

                return (
                  <button key={v.id} type="button"
                    disabled={busy}
                    onClick={() => { setSelectedId(v.id); setUseCombo(false); }}
                    style={{
                      border: `2px solid ${borderColor}`,
                      background: bgColor, borderRadius: 8,
                      padding: '10px 10px 8px', textAlign: 'left', cursor: busy ? 'not-allowed' : 'pointer',
                      transition: 'border-color .12s, background .12s',
                      opacity: busy ? 0.55 : 1,
                    }}>
                    <div style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: 13, color: 'var(--ink)', letterSpacing: '.04em' }}>
                      {v.targa || v.id}
                    </div>
            ...
            {/* Combo automatica se nessun mezzo copre tutto */}
            {smartCombo && !smartCombo.impossible && (
              <div style={{ marginTop: 12, padding: '12px 14px', borderRadius: 8,
                border: `2px solid ${useCombo ? '#1f5d83' : '#c88a2e'}`,
                background: useCombo ? '#e8f2f9' : '#fffbf0' }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#c88a2e', marginBottom: 6 }}>
                  🧩 Nessun mezzo copre i {nNoleggio} giorni — combinazione automatica
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 8 }}>
                  {smartCombo.map((seg, i) => (
                    <span key={i} style={{ fontSize: 11, background: 'rgba(31,93,131,.1)', color: '#1f5d83',
                      borderRadius: 4, padding: '2px 8px', fontFamily: 'monospace', fontWeight: 600 }}>
                      {seg.vehicleId}
                      <span style={{ fontWeight: 400, color: 'var(--muted)', marginLeft: 4 }}>
                        {formatDate(seg.dal)}→{formatDate(seg.al)}
                      </span>
                      {i < smartCombo.length - 1 && <span style={{ marginLeft: 4 }}>›</span>}
                    </span>
                  ))}
                </div>
                <button type="button" onClick={() => { setUseCombo(true); setSelectedId(''); }}
                  style={{ fontSize: 11, padding: '5px 14px', borderRadius: 5,
                    border: 'none', background: useCombo ? '#1f5d83' : '#c88a2e',
                    color: 'white', fontWeight: 700, cursor: 'pointer' }}>
                  {useCombo ? '✓ Combinazione selezionata' : 'Usa questa combinazione'}
                </button>
              </div>
            )}
            {smartCombo?.impossible && (
              <div style={{ marginTop: 10, padding: '10px 14px', borderRadius: 8,
                border: '1px solid #e0c0c0', background: '#fff5f5', fontSize: 12, color: '#c85050' }}>
                ⚠ Nessuna combinazione possibile per questo periodo. Verifica le prenotazioni esistenti.
              </div>
            )}

            {/* Avviso mezzo già assegnato */}
            {preno.vehicleId && !selectedId && !useCombo && (
              <div style={{ marginTop: 8, fontSize: 12, color: 'var(--muted)', fontStyle: 'italic' }}>
                Mezzo precedentemente assegnato: <strong style={{ fontFamily: 'monospace' }}>{preno.vehicleId}</strong> — selezionalo dalla griglia o scegli un altro.
              </div>
            )}

            {/* Stato selezione corrente */}
            {selectedId && !useCombo && selVehicle && (
              <div style={{ marginTop: 10, padding: '8px 12px', borderRadius: 7,
                background: coversAll ? 'rgba(46,110,62,.08)' : 'rgba(200,138,46,.1)',
                border: `1px solid ${coversAll ? '#4a9e5c' : '#c88a2e'}`,
                fontSize: 12 }}>
                {coversAll
                  ? <span style={{ color: '#2e6e3e', fontWeight: 600 }}>✓ <strong style={{ fontFamily: 'monospace' }}>{selectedId}</strong> disponibile per tutti i {nNoleggio} giorni</span>
                  : <span style={{ color: '#c88a2e', fontWeight: 600 }}>⚠ <strong style={{ fontFamily: 'monospace' }}>{selectedId}</strong> libero solo {selVehicle.freeScore}/{nNoleggio} giorni — considera la combo automatica</span>
                }
              </div>
            )}
          </div>

          {/* ── Sezione: Check-out iniziale ── */}
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--muted)', textTransform: 'uppercase',
              letterSpacing: '.08em', marginBottom: 10 }}>
              Stato alla partenza
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <div>
                ...
                <input type="number" min={0} step={1} placeholder="es. 12430"
                  value={kmPartenza} onChange={e => setKmPartenza(e.target.value)}
                  style={inp} />
              </div>

              {/* Data consegna (read-only) */}
              <div>
                <label style={{ fontSize: 12, color: 'var(--muted)', display: 'block', marginBottom: 4 }}>
                  Data consegna
                </label>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {FUEL_OPTS.map(opt => (
                  <button key={opt.val} type="button"
                    onClick={() => setCarburante(carburante === opt.val ? '' : opt.val)}
                    style={{
                      flex: 1, padding: '8px 4px', borderRadius: 6, fontSize: 11, fontWeight: 700,
                      cursor: 'pointer',
                      border: carburante === opt.val ? `2px solid ${opt.color}` : '1px solid var(--border)',
                      background: carburante === opt.val ? opt.color + '22' : 'var(--surface)',
                      color: carburante === opt.val ? opt.color : 'var(--muted)',
                    }}>
                    {opt.label}
                  </button>
                ))}
            ...
            <div style={{ marginTop: 14 }}>
              ...
              <textarea rows={2} placeholder="Graffi presenti, accessori consegnati, altro…"
                value={noteConsegna} onChange={e => setNoteConsegna(e.target.value)}
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', paddingTop: 4,
            ...
            <button type="button" onClick={onClose}
              style={{ padding: '9px 20px', borderRadius: 7, border: '1px solid var(--border)',
                background: 'transparent', color: 'var(--ink-2)', fontSize: 13, cursor: 'pointer' }}>
              Annulla
            </button>
            <button type="button" onClick={doConfirm} disabled={!canConfirm}
              style={{ padding: '9px 24px', borderRadius: 7, border: 'none',
                background: canConfirm ? '#2e6e3e' : 'var(--muted)',
...
// ═══════════════════════════════════════════════════════════════════
// RICONSEGNA MODAL — registra km finale, carburante, danni al rientro
// ═══════════════════════════════════════════════════════════════════
function RiconsegnaModal({ preno, onConfirm, onClose }) {
  const [kmRientro, setKmRientro] = useState(preno.kmRientro ?? '');
  const [carburante, setCarburante] = useState(preno.carburante || '');
  const [danniCheck, setDanniCheck] = useState({
    carrozzeria: false, cristalli: false, pneumatici: false,
    interni: false, specchietti: false, altro: false,
  });
  const [noteDanni, setNoteDanni] = useState(preno.noteDanni || '');
  const [noteRientro, setNoteRientro] = useState(preno.noteRientro || '');

  const kmPartenza = preno.kmPartenza;
  const kmPercorsi = (kmRientro !== '' && kmPartenza != null)
    ? Math.max(0, Number(kmRientro) - Number(kmPartenza))
    : null;
  const hasDanni = Object.values(danniCheck).some(Boolean);

  const CARBURANTE_OPTIONS = [
    { val: 'vuoto',  label: '▭ Vuoto',   color: '#c85050' },
    { val: '1/4',    label: '¼',          color: '#e07030' },
    { val: '1/2',    label: '½',          color: '#c88a2e' },
    { val: '3/4',    label: '¾',          color: '#4a7a3a' },
    { val: 'pieno',  label: '▮ Pieno',   color: '#2a6e20' },
  ];

  const DANNI_LABELS = {
    carrozzeria: 'Carrozzeria', cristalli: 'Cristalli', pneumatici: 'Pneumatici',
    interni: 'Interni', specchietti: 'Specchietti', altro: 'Altro',
  };

  const doConfirm = () => {
    const danni = Object.entries(danniCheck).filter(([, v]) => v).map(([k]) => DANNI_LABELS[k]);
    onConfirm({
      kmRientro:   kmRientro !== '' ? Number(kmRientro) : null,
      carburanteRientro: carburante || null,
      danniRiscontrati:  danni.length ? danni : null,
      noteDanni:   hasDanni ? noteDanni : '',
      noteRientro: noteRientro || '',
    });
  };
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
      <div style={{ background: 'var(--surface)', borderRadius: 12, padding: 28, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 8px 40px rgba(0,0,0,.18)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
          <div>
            <div className="serif" style={{ fontSize: 20, fontWeight: 600 }}>📍 Riconsegna</div>
            <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>
              {[preno.clienteCognome, preno.clienteNome].filter(Boolean).join(' ')}
              {preno.vehicleLabel && <> · <span className="mono font-semibold">{preno.vehicleLabel}</span></>}
            </div>
          </div>
        ...
        <div style={{ marginBottom: 18 }}>
          <label style={{ display: 'block', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--ink-2)', marginBottom: 8 }}>
            Chilometri al rientro
          </label>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <input
              type="number" min="0" value={kmRientro}
              onChange={e => setKmRientro(e.target.value)}
              placeholder="es. 12450"
              style={{ width: 160, padding: '9px 12px', border: '1.5px solid var(--border)', borderRadius: 6, fontSize: 16, fontFamily: 'var(--font-mono)', outline: 'none', color: 'var(--ink)' }}
            />
            {kmPercorsi !== null && (
              <div style={{ fontSize: 13, color: 'var(--muted)' }}>
                <span style={{ fontWeight: 600, color: 'var(--ink)' }}>{kmPercorsi}</span> km percorsi
                {kmPartenza != null && <span style={{ marginLeft: 6 }}>(da {kmPartenza})</span>}
              </div>
            )}
          </div>
        </div>

        {/* CARBURANTE AL RIENTRO */}
        <div style={{ marginBottom: 18 }}>
          <label style={{ display: 'block', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--ink-2)', marginBottom: 8 }}>
            Carburante al rientro
            {preno.carburante && <span style={{ fontWeight: 400, color: 'var(--muted)', marginLeft: 8, textTransform: 'none' }}>
              (alla partenza: {preno.carburante})
            </span>}
          </label>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {CARBURANTE_OPTIONS.map(opt => (
              <button key={opt.val} type="button"
                onClick={() => setCarburante(v => v === opt.val ? '' : opt.val)}
                style={{ padding: '7px 14px', borderRadius: 6, border: `2px solid ${carburante === opt.val ? opt.color : 'var(--border)'}`, background: carburante === opt.val ? opt.color + '20' : 'transparent', color: carburante === opt.val ? opt.color : 'var(--ink-2)', fontWeight: 600, fontSize: 13, cursor: 'pointer', transition: 'all .15s' }}>
                {opt.label}
              </button>
            ))}
          </div>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: hasDanni ? 10 : 0 }}>
            {Object.entries(DANNI_LABELS).map(([key, label]) => (
              <button key={key} type="button"
                onClick={() => setDanniCheck(d => ({ ...d, [key]: !d[key] }))}
                style={{ padding: '6px 13px', borderRadius: 6, border: `2px solid ${danniCheck[key] ? 'var(--accent)' : 'var(--border)'}`, background: danniCheck[key] ? 'var(--accent-soft)' : 'transparent', color: danniCheck[key] ? 'var(--accent-deep)' : 'var(--ink-2)', fontWeight: danniCheck[key] ? 700 : 400, fontSize: 12, cursor: 'pointer', transition: 'all .15s' }}>
                {danniCheck[key] ? '⚠ ' : ''}{label}
              </button>
            ))}
          </div>
          {hasDanni && (
            <textarea
              value={noteDanni} onChange={e => setNoteDanni(e.target.value)}
              placeholder="Descrivi i danni riscontrati…"
              rows={2}
              style={{ width: '100%', padding: '8px 10px', border: '1.5px solid var(--accent)', borderRadius: 6, fontSize: 13, resize: 'vertical', outline: 'none', color: 'var(--ink)', background: 'var(--bg)', marginTop: 4 }}
            />
          )}
          {!hasDanni && (
            <div style={{ fontSize: 12, color: 'var(--success)', fontStyle: 'italic', marginTop: 4 }}>
              ✓ Nessun danno selezionato
            </div>
          )}
        </div>
        ...
        <div style={{ marginBottom: 22 }}>
          <label style={{ display: 'block', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--ink-2)', marginBottom: 6 }}>
            Note operatore
          </label>
          <textarea
            value={noteRientro} onChange={e => setNoteRientro(e.target.value)}
            placeholder="Note interne sul rientro…"
            rows={2}
            style={{ width: '100%', padding: '8px 10px', border: '1px solid var(--border)', borderRadius: 6, fontSize: 13, resize: 'vertical', outline: 'none', color: 'var(--ink)', background: 'var(--bg)' }}
          />
        </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button type="button" onClick={doConfirm}
            style={{ flex: 1, padding: '11px 0', borderRadius: 6, border: 'none', background: hasDanni ? 'var(--accent)' : '#2a6e20', color: '#fff', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
            {hasDanni ? '⚠ Conferma rientro con danni' : '✓ Conferma rientro'}
          </button>
          <button type="button" onClick={onClose}
            style={{ padding: '11px 18px', borderRadius: 6, border: '1px solid var(--border)', background: 'transparent', color: 'var(--ink-2)', fontWeight: 500, fontSize: 13, cursor: 'pointer' }}>
            Annulla
          </button>
        </div>
      </div>
...
function PreventiviPage({ setPage, setPrenotazioniPrefill, listino: listinoProps, fleet, rentmeVehicles, prenotazioni, pushToast }) {
  // Merge: recupera 'categoria' dal LISTINO costante se mancante nel dato persistito
...
function calcAvailability(dal, al, rentmeVehicles, prenotazioni, fleet) {
  ...
  (prenotazioni || []).forEach(b => {
    ...
    const cat = b.vehicleLabel.split('·')[0].trim() || b.vehicleLabel;
    if (!catMap[cat]) catMap[cat] = { id: b.vehicleId || cat, nome: cat, tipo: b.vehicleType || '', total: 0, booked: 0 };
    if (b.vehicleId && !catMap[cat].known) { catMap[cat].total++; catMap[cat].known = true; }
  ...
  const sync = useCallback(async () => {
    ...
    try {
      ...
      const mapped = veicoli.map(v => {
        const rmCode  = (v.targa || '').trim(); // codice RentMe es. "mehari 130"
      ...
      const rentmeRows = [];
      veicoli.forEach(v => {
        const rmCode  = (v.targa || '').trim();
        const targaId = rentmeToTarga[rmCode.toLowerCase()] || rentmeToTarga[rmCode.trim().split(' ').pop()?.toLowerCase()] || rmCode.toUpperCase();
        ...
        (v.dateImpegno || []).forEach(di => {
          ...
          if (p.length >= 2 && p[0] && p[1]) {
            ...
            rentmeRows.push({
              ...
              note:          (p[5]||'').trim(),
              rentmeTarga:   targaId,
              rentmeCode:    rmCode,   // codice originale RentMe
            });
          }
        });
      });
  ...
  return (
    <div className="edo-chiusura-modal" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '40px 20px', overflowY: 'auto' }}>
      <div style={{ background: 'var(--bg)', borderRadius: 10, width: '100%', maxWidth: 520, padding: '28px 30px', boxShadow: '0 8px 40px rgba(0,0,0,.22)' }}>
        ...
        <div id="chiusura-print-area">

          {/* Solo per stampa */}
          <div className="chiusura-print-header" style={{ display: 'none', textAlign: 'center', marginBottom: 14 }}>
            <div style={{ fontSize: 15, fontWeight: 700, letterSpacing: '.04em' }}>EDONOLEGGIO · LAMPEDUSA</div>
            <div style={{ fontSize: 12, color: '#555', marginTop: 2 }}>Chiusura cassa del {fmtDate(selectedDate)}</div>
            <hr style={{ margin: '8px 0', borderColor: '#ccc' }} />
          </div>

          {/* Riepilogo top */}
          <div style={{ background: 'var(--surface-2)', borderRadius: 8, padding: '12px 16px', marginBottom: 16, borderLeft: '3px solid var(--ink)' }}>
          ...
          {/* Per tipo */}
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--muted)', marginBottom: 7 }}>Per tipo</div>
            <div style={{ border: '1px solid var(--border)', borderRadius: 7, overflow: 'hidden' }}>
              {Object.entries(CASSA_TIPI).map(([k, v], i, arr) => {
                const val = perTipo[k] || 0;
                const isRimborso = k === 'rimborso';
                return (
                  <div key={k} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 14px', background: i % 2 === 0 ? 'var(--bg)' : 'var(--surface-2)', borderBottom: i < arr.length-1 ? '1px solid var(--border)' : 'none' }}>
                    <span className={v.pill} style={{ fontSize: 10, padding: '2px 7px', borderRadius: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.04em' }}>{v.label}</span>
                    <span style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: 14, color: isRimborso ? '#c0392b' : (val > 0 ? '#27ae60' : 'var(--muted)') }}>
                      {isRimborso ? '−' : '+'}{fmtEur(val)}
                    </span>
                  </div>
                );
              })}
          ...
          {/* Per metodo */}
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--muted)', marginBottom: 7 }}>Per metodo di pagamento</div>
            <div style={{ border: '1px solid var(--border)', borderRadius: 7, overflow: 'hidden' }}>
              {Object.entries(CASSA_METODI).map(([k, m], i, arr) => {
                const val = perMetodo[k] || 0;
                return (
                  <div key={k} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 14px', background: i % 2 === 0 ? 'var(--bg)' : 'var(--surface-2)', borderBottom: i < arr.length-1 ? '1px solid var(--border)' : 'none' }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: m.color }}>{m.icon} {m.label}</span>
                    <span style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: 14, color: val > 0 ? m.color : 'var(--muted)' }}>{fmtEur(val)}</span>
                  </div>
                );
              })}
            </div>
          ...
          {/* Netto finale */}
          <div style={{ background: netto >= 0 ? '#27ae6014' : '#c0392b14', border: `1px solid ${netto >= 0 ? '#27ae6044' : '#c0392b44'}`, borderRadius: 7, padding: '11px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--ink)' }}>Totale netto</span>
            <span style={{ fontFamily: 'monospace', fontSize: 18, fontWeight: 700, color: netto >= 0 ? '#27ae60' : '#c0392b' }}>{fmtEur(netto)}</span>
          </div>

          {/* Footer stampa */}
          <div className="chiusura-print-footer" style={{ display: 'none', marginTop: 20, textAlign: 'center', fontSize: 10, color: '#999', borderTop: '1px solid #ddd', paddingTop: 8 }}>
            Stampato il {new Date().toLocaleDateString('it-IT')} · {new Date().toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' })}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 20, justifyContent: 'flex-end' }}>
          <button type="button" onClick={onClose}
            style={{ padding: '9px 18px', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 5, fontSize: 13, cursor: 'pointer', color: 'var(--ink-2)' }}>
            Chiudi
          </button>
      ...
      <style>{`
        @media print {
          ...
          .edo-chiusura-modal > div {
            box-shadow: none !important; border: none !important;
            max-width: 100% !important; width: 100% !important; padding: 10px !important;
          }
          .chiusura-print-header { display: block !important; }
          .chiusura-print-footer { display: block !important; }
          button, input[type="date"], label { display: none !important; }
          [style*="display: flex"][style*="gap: 10"][style*="marginTop: 20"] { display: none !important; }
          div[style*="justifyContent: flex-end"] { display: none !important; }
        }
      `}</style>
    </div>
  );
...
function RegistroCassaPage({ cassa, setCassa, prenotazioni, customers, operator, pushToast }) {
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1000, margin: '0 auto' }}>
      ...
      {showChiusura && (
        <ChiusuraGiornalieraModal
          cassa={cassa}
          initialDate={today}
          onClose={() => setShowChiusura(false)}
        />
      )}
    </div>
  );
  ...
  function printConsegne(items, titolo) {
    const rows = items.map(p => `
      <tr>
        ...
        <td>${p.dal || '—'} → ${p.al || '—'}</td>
        <td>${p.stato || '—'}</td>
        <td>${p.note || p.noteCliente || ''}</td>
      </tr>`).join('');
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8">
      <title>${titolo} · Edonoleggio</title>
      <style>
        body { font-family: Georgia, serif; font-size: 13px; color: #222; padding: 24px; }
    ...
    <div style={{ padding: '28px 32px', maxWidth: 860, margin: '0 auto' }}>
      ...
      {(() => {
        ...
        const nGiorni = walkInDal && walkInAl
          ...
          : 1;
        const setNDays = (n) => {
          const d = new Date(walkInDal);
          d.setDate(d.getDate() + n - 1);
          setWalkInAl(d.toISOString().slice(0, 10));
        };
        return (
          <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                {[1, 2, 3, 4, 5].map(n => (
                  <button key={n} type="button" onClick={() => setNDays(n)}
                    style={{
                      fontSize: 11, padding: '4px 10px', borderRadius: 12,
                      border: `1px solid ${nGiorni === n ? '#7c4dff' : 'var(--border)'}`,
                      background: nGiorni === n ? '#7c4dff' : 'transparent',
                      color: nGiorni === n ? '#fff' : 'var(--muted)',
                      cursor: 'pointer', fontWeight: nGiorni === n ? 700 : 400,
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10, marginTop: 20 }}>
        <SectionTitle icon="📥" label="Rientri oggi" count={rientri.length} color="#6a3d8f" noMargin />
        {rientri.length > 0 && (
          <button type="button" onClick={() => printConsegne(rientri, 'Rientri oggi')}
            style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, padding: '4px 10px',
              borderRadius: 6, border: '1px solid #6a3d8f', background: 'transparent', color: '#6a3d8f',
...
function CalendarioFlottaPage({ prenotazioni, fleet, rentmeVehicles, setPage, setPrenotazioniPrefill }) {
  ...
  // Tipi unici per filtro — canonicalTipo normalizza moto→scooter, bicicletta→ebike
  const tipiDisponibili = useMemo(() =>
    [...new Set(fleetList.map(v => canonicalTipo(v)).filter(t => t && t !== 'bici'))].sort(),
    [fleetList]
  );

  const fleetFiltered = useMemo(() =>
    tipoFilter ? fleetList.filter(v => canonicalTipo(v) === tipoFilter) : fleetList,
  ...
  const cellMap = useMemo(() => {
    const m = {};
    prenoList.forEach(p => {
      if (p.vehicleSchedule && p.vehicleSchedule.length > 1) {
          ...
          days.forEach(d => {
            if (seg.dal <= d && seg.al >= d) {
              m[`${seg.vehicleId}|${d}`] = {
                ...
                _segDal:     seg.dal,
                _segAl:      seg.al,
                _isFirstSeg: segIdx === 0,
                _isLastSeg:  segIdx === p.vehicleSchedule.length - 1,
              };
            }
          });
        });
          ...
          {fleetFiltered.length === 0 ? (
            ...
            <div key={v.id} style={{ display: 'flex', alignItems: 'stretch',
              ...
              {days.map(d => {
                ...
                const isStart = preno && (segDal ? segDal === d : preno.dal === d);
...
export default function App() {
  ...
  // Importa flotta da EDOX/RentMe — sincronizza fleet locale con i veicoli RentMe reali
  ...
  const requestImportFleetFromRentMe = useCallback(() => {
    setModal({
      ...
      message: <>Sostituisce <strong>tutta</strong> la flotta attuale con i {rentmeVehicles?.length || 0} veicoli presenti su EDOX/RentMe. La flotta corrente verrà sovrascritta. <strong>Azione irreversibile.</strong></>,
  ...
  // ── LOGIN GATE ────────────────────────────────────────────────────
  // Se non c'è sessione attiva, mostra solo la pagina di login.
  // Nessun dato dell'app è visibile o raggiungibile senza autenticazione.
  if (!sessionUser) {
    return (
      <>
        <Styles />
...
// LOGIN PAGE
// ═══════════════════════════════════════════════════════════════════
function LoginPage({ appUsers, onLogin }) {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    setError('');
    // Usa lista custom se configurata, altrimenti default built-in
    ...
    const session = { username: user.username, role: user.role, nome: user.nome, loginAt: new Date().toISOString() };
    try { sessionStorage.setItem(APP_SESSION_KEY, JSON.stringify(session)); } catch {}
    onLogin(session);
  };

  return (
    <div className="pratica-app" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
      <div style={{ width: '100%', maxWidth: 380, padding: '0 20px' }}>

        {/* Branding */}
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          <div style={{ width: 68, height: 68, borderRadius: '50%', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px', boxShadow: '0 4px 18px rgba(200,52,52,.25)' }}>
            <Anchor className="w-7 h-7" style={{ color: '#fff' }} />
          </div>
          <div className="serif" style={{ fontSize: 30, fontWeight: 600, color: 'var(--ink)', letterSpacing: '-0.02em', lineHeight: 1.1 }}>Pratica</div>
          <div style={{ fontSize: 13, color: 'var(--muted)', marginTop: 5 }}>Edonoleggio · Lampedusa</div>
        </div>

        {/* Card form */}
        <form onSubmit={handleSubmit} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '28px 28px 24px', boxShadow: '0 3px 20px rgba(0,0,0,0.07)' }}>
          <div style={{ marginBottom: 15 }}>
            <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: 'var(--ink-2)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>
              Username
            </label>
            <input
              type="text"
              ...
              style={{ width: '100%', padding: '10px 12px', border: `1.5px solid ${error ? 'var(--accent)' : 'var(--border)'}`, borderRadius: 6, fontSize: 14, outline: 'none', background: 'var(--bg)', color: 'var(--ink)', transition: 'border-color .15s' }}
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ position: 'relative' }}>
              <input
                ...
                style={{ width: '100%', padding: '10px 40px 10px 12px', border: `1.5px solid ${error ? 'var(--accent)' : 'var(--border)'}`, borderRadius: 6, fontSize: 14, outline: 'none', background: 'var(--bg)', color: 'var(--ink)', transition: 'border-color .15s' }}
                placeholder="••••••••"
                onFocus={e => e.target.style.borderColor = 'var(--ink)'}
                onBlur={e => e.target.style.borderColor = error ? 'var(--accent)' : 'var(--border)'}
              />
              <button type="button" onClick={() => setShowPwd(s => !s)} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', padding: 3, borderRadius: 3 }} aria-label={showPwd ? 'Nascondi password' : 'Mostra password'}>
          ...
          {error && (
            <div style={{ background: 'var(--accent-soft)', color: 'var(--accent-deep)', fontSize: 12, fontWeight: 500, padding: '8px 11px', borderRadius: 6, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 7 }}>
          ...
          <button
            ...
            style={{ width: '100%', padding: '11px 0', fontSize: 14, fontWeight: 600, border: 'none', borderRadius: 6, cursor: loading ? 'wait' : 'pointer', letterSpacing: '0.01em' }}
          >
            {loading ? 'Accesso in corso…' : 'Accedi →'}
          </button>
        </form>

        <div style={{ textAlign: 'center', marginTop: 18, fontSize: 11, color: 'var(--muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
          <Lock className="w-3 h-3" />
          <span>Sessione attiva fino alla chiusura del browser</span>
        </div>
      </div>
    </div>
  );
    ...
    <header className="border-b px-8 py-3 flex items-center gap-3" style={{ borderColor: 'var(--border)', background: 'var(--bg)' }}>
      ...
      {onLogout && (
        <button
          type="button"
          onClick={() => {
            if (window.confirm(`Uscire dalla sessione di ${sessionUser?.username || 'utente'}?`)) onLogout();
          }}
          className="btn-ghost px-2 py-1.5 rounded text-xs flex items-center gap-1.5 border"
          style={{ borderColor: 'var(--border)', color: 'var(--muted)' }}
          title="Esci — chiude la sessione corrente"
          aria-label="Logout"
        >
...
function FleetPage({ fleet, prenotazioni, admin, onAddVehicle, onEditVehicle, onDeleteVehicle, onImportCSV, scadenze, setScadenze, fermiFlotta, setFermiFlotta, rentmeVehicles }) {
  ...
  const filters = [
    ...
    ...(counts.bici > 0 ? [{ id: 'bici', label: 'Bici', n: counts.bici }] : []),
  ];
...
// ─── Fermi Flotta Section ──────────────────────────────────────────
function FermiFlottaSection({ fleet, fermiFlotta, setFermiFlotta }) {
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ vehicleId: '', dal: todayISO(), al: '', motivo: '' });
  const today = todayISO();

  const activeFermi = fermiFlotta.filter(f => f.al >= today);
  const pastFermi   = fermiFlotta.filter(f => f.al < today);
  ...
  const addFermo = () => {
    if (!form.vehicleId || !form.dal || !form.al || form.al < form.dal) return;
    setFermiFlotta(prev => [...(prev || []), { id: `fermo-${Date.now()}`, ...form }]);
    setForm({ vehicleId: '', dal: todayISO(), al: '', motivo: '' });
    setShowForm(false);
  };

  const removeFermo = (id) => {
    if (!window.confirm('Rimuovere questo fermo?')) return;
    setFermiFlotta(prev => (prev || []).filter(f => f.id !== id));
  };

  const inp = { padding: '8px 10px', border: '1px solid var(--border)', borderRadius: 5, fontSize: 13, outline: 'none', color: 'var(--ink)', background: 'var(--bg)' };
  ...
  return (
    <section style={{ marginTop: 28, padding: '20px 22px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8 }}>
      ...
      {/* Form aggiunta */}
      {showForm && (
        <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 7, padding: '16px 18px', marginBottom: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
            <div>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: 'var(--ink-2)', marginBottom: 4 }}>Veicolo</label>
              <select style={{ ...inp, width: '100%' }} value={form.vehicleId} onChange={e => setForm(f => ({ ...f, vehicleId: e.target.value }))}>
                <option value="">— seleziona —</option>
                {[...fleet].sort((a, b) => (a.targa || a.id).localeCompare(b.targa || b.id)).map(v => (
                  <option key={v.id} value={v.id}>{v.targa || v.id}{v.modello ? ' · ' + v.modello : ''}</option>
                ))}
              </select>
            </div>
            <div>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: 'var(--ink-2)', marginBottom: 4 }}>Motivo</label>
            <input type="text" style={{ ...inp, width: '100%' }} value={form.motivo} onChange={e => setForm(f => ({ ...f, motivo: e.target.value }))} placeholder="Officina, revisione, tagliando, incidente…" />
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="button" onClick={addFermo} disabled={!form.vehicleId || !form.dal || !form.al}
              style={{ padding: '8px 18px', borderRadius: 5, border: 'none', background: 'var(--accent)', color: '#fff', fontWeight: 600, fontSize: 13, cursor: 'pointer', opacity: (!form.vehicleId || !form.al) ? 0.4 : 1 }}>
              Conferma
            </button>
            <button type="button" onClick={() => setShowForm(false)}
              style={{ padding: '8px 14px', borderRadius: 5, border: '1px solid var(--border)', background: 'transparent', fontSize: 12, cursor: 'pointer', color: 'var(--ink-2)' }}>
              Annulla
            </button>
          </div>
        </div>
      ...
      {/* Lista fermi attivi */}
      {activeFermi.length === 0 && !showForm && (
        <div style={{ fontSize: 13, color: 'var(--muted)', fontStyle: 'italic', textAlign: 'center', padding: '12px 0' }}>
          Nessun fermo programmato attivo.
        </div>
      )}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {activeFermi.map(f => (
          <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 14px', background: 'var(--warning-soft)', border: '1px solid var(--border)', borderRadius: 6 }}>
            <AlertTriangle className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--warning)' }} />
            <div style={{ flex: 1 }}>
              <span className="mono font-semibold" style={{ fontSize: 13 }}>{vehicleLabel(f.vehicleId)}</span>
              <span style={{ fontSize: 12, color: 'var(--muted)', marginLeft: 10 }}>
                {formatDate(f.dal)} → {formatDate(f.al)}
              </span>
              {f.motivo && <span style={{ fontSize: 12, color: 'var(--ink-2)', marginLeft: 8 }}>· {f.motivo}</span>}
            </div>
            <button type="button" onClick={() => removeFermo(f.id)}
        ...
        {pastFermi.length > 0 && (
          <details style={{ marginTop: 8 }}>
            <summary style={{ fontSize: 11, color: 'var(--muted)', cursor: 'pointer' }}>
              {pastFermi.length} fermi passati (archivio)
            </summary>
            <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 4 }}>
              {pastFermi.map(f => (
                <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 12px', background: 'var(--surface-2)', borderRadius: 5, opacity: 0.7 }}>
                  <span className="mono" style={{ fontSize: 12 }}>{vehicleLabel(f.vehicleId)}</span>
                  <span style={{ fontSize: 11, color: 'var(--muted)' }}>{formatDate(f.dal)} → {formatDate(f.al)}</span>
                  {f.motivo && <span style={{ fontSize: 11, color: 'var(--muted)' }}>· {f.motivo}</span>}
                  <button type="button" onClick={() => removeFermo(f.id)}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', marginLeft: 'auto', padding: 2 }}>
                    <X className="w-3.5 h-3.5" />
                  </button>
                </div>
              ))}
...
// ═══════════════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════════════
// SECURITY SECTION — gestione utenti e password (solo in Admin mode)
// ═══════════════════════════════════════════════════════════════════
function SecuritySection({ appUsers, setAppUsers, onLogout, pushToast }) {
  ...
  const saveEdit = () => {
    if (!form.username.trim()) return pushToast?.({ tone: 'error', title: 'Username obbligatorio' });
    if (form.password && form.password !== form.password2) return pushToast?.({ tone: 'error', title: 'Le password non coincidono' });
    setAppUsers(prev => (prev || APP_DEFAULT_USERS).map(u =>
      u.id === editing ? {
        ...u,
    ...
    pushToast?.({ tone: 'success', title: 'Utente aggiornato' });
    cancel();
  };

  const saveAdd = () => {
    if (!form.username.trim()) return pushToast?.({ tone: 'error', title: 'Username obbligatorio' });
    if (!form.password) return pushToast?.({ tone: 'error', title: 'Password obbligatoria' });
    if (form.password !== form.password2) return pushToast?.({ tone: 'error', title: 'Le password non coincidono' });
    const existing = (appUsers || APP_DEFAULT_USERS);
  ...
  const deleteUser = (u) => {
    if (users.length <= 1) return pushToast?.({ tone: 'error', title: 'Impossibile eliminare l\'unico utente' });
    if (!window.confirm(`Eliminare l'utente "${u.username}"?`)) return;
    setAppUsers(prev => (prev || APP_DEFAULT_USERS).filter(x => x.id !== u.id));
    pushToast?.({ tone: 'warning', title: 'Utente eliminato', message: u.username });
  };

  const inputStyle = { width: '100%', padding: '8px 10px', border: '1px solid var(--border)', borderRadius: 5, fontSize: 13, outline: 'none', background: 'var(--bg)', color: 'var(--ink)' };

  return (
    <section className="card-paper p-6 mt-4" aria-labelledby="security-heading">
      <div className="flex items-center gap-2 mb-1">
        <Shield className="w-4 h-4" style={{ color: 'var(--sea)' }} />
        <h3 id="security-heading" className="serif text-lg font-medium">Sicurezza · Accesso</h3>
      </div>
      <p className="text-xs mb-4" style={{ color: 'var(--muted)' }}>
        Utenti che possono accedere all'app. Le credenziali sono salvate solo su questo dispositivo.
      </p>

      {/* Lista utenti */}
      <div className="space-y-2 mb-4">
        {users.map(u => (
          <div key={u.id} className="flex items-center justify-between p-3 rounded border" style={{ borderColor: 'var(--border)', background: 'var(--surface-2)' }}>
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold" style={{ background: u.role === 'admin' ? 'var(--accent)' : 'var(--sea)', color: '#fff' }}>
                {u.username[0].toUpperCase()}
              </div>
              <div>
                <div className="font-medium text-sm">{u.username}</div>
                <div className="text-[11px]" style={{ color: 'var(--muted)' }}>{u.role === 'admin' ? 'Amministratore' : 'Operatore'}</div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => openEdit(u)} className="btn-ghost px-2 py-1 rounded text-xs border" style={{ borderColor: 'var(--border)' }}>
                <Pencil className="w-3 h-3" />
              </button>
              <button type="button" onClick={() => deleteUser(u)} className="btn-ghost px-2 py-1 rounded text-xs border" style={{ borderColor: 'var(--border)', color: 'var(--accent)' }}>
      ...
      {(editing || addMode) && (
        <form onSubmit={e => { e.preventDefault(); addMode ? saveAdd() : saveEdit(); }}
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            ...
            <div />
            <div>
              <label className="block text-[11px] font-semibold mb-1" style={{ color: 'var(--ink-2)' }}>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        {!addMode && !editing && (
          <button type="button" onClick={openAdd} className="btn-ghost px-3 py-1.5 rounded text-xs font-semibold border flex items-center gap-1.5" style={{ borderColor: 'var(--sea)', color: 'var(--sea)' }}>
            <Plus className="w-3.5 h-3.5" /> Aggiungi utente
          </button>
        )}
        {onLogout && (
          <button type="button" onClick={() => { if (window.confirm('Esci e torna alla pagina di login?')) onLogout(); }} className="btn-ghost px-3 py-1.5 rounded text-xs font-semibold border flex items-center gap-1.5" style={{ borderColor: 'var(--border)', color: 'var(--ink-2)' }}>
            <Power className="w-3.5 h-3.5" /> Esci (logout)
          </button>
        )}
      </div>
    </section>
...
function SettingsPage({ operator, operators, cargosConfig, admin, backendStatus, lastCheck, apiBaseUrl, syncStatus, agency, onSyncAll, onExportBackup, onImportBackup, pushToast, onAddOperator, onEditOperator, onDeleteOperator, onEditCargos, onEditApiBase, onEditAgency, onResetCustomers, onResetContracts, onResetEverything, onImportFleetFromRentMe, customers, contracts, rentmeConfig, setRentmeConfig, rentmeSync, rentmeVehicles, prenotazioni, onImportStorico, appUsers, setAppUsers, onLogout }) {
  ...
  return (
    <div>
      ...
      {/* ── SICUREZZA ACCESSO ────────────────────────────────────────── */}
      {admin && setAppUsers && (
        <SecuritySection appUsers={appUsers} setAppUsers={setAppUsers} onLogout={onLogout} pushToast={pushToast} />
      )}
